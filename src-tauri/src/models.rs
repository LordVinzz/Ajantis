...
#[tauri::command]
pub(crate) async fn set_model(state: tauri::State<'_, Arc<AppState>>, model: String) -> Result<bool, String> {
    if !model.is_empty() {
...
pub(crate) async fn load_model_internal(config: &AgentLoadConfig, model_key: &str) -> Result<(), String> {
